...
void Leg::setAngles(float q1, float q2, float q3) {
    if (!checkAngleBounds(q1, q2, q3)) return;
...
// Prints error message if any angle is out of bounds.
bool Leg::checkAngleBounds(float q1, float q2, float q3) const {
    if (q1 < -0.5236f || q1 > 2.0944f) {
        std::cerr << "q1 out of bounds : " << q1 << " (ch " << joints[0].getChannel() << ")\n";
        return false;
    } 
    if (q2 < -1.309f || q2 > 1.309f) {
        std::cerr << "q2 out of bounds : " << q2 << " (ch " << joints[1].getChannel() << ")\n";
        return false;
    } 
    if (q3 < 0 || q3 > 2.618f) {
        std::cerr << "q3 out of bounds : " << q3 << " (ch " << joints[2].getChannel() << ")\n";
        return false;
    } 
    if (-q2 + q3 > 3.0f) {
        std::cerr << "q3 - q2 out of bounds : " << -q2 + q3 << " (ch " << joints[1].getChannel() << " + " << joints[2].getChannel() << ")\n";
        return false;
    }
    return true;
}
